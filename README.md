...
$ mpttune generate --interactive --model mpt-7b-storyteller-4bit --weights mpt-7b-storywriter-4bit-128g.safetensors --max_new_tokens=600 --use_cache --do_sample --prompt "$book"
...
```
You may now finetune the `mpt-7b-storyteller-4bit` model on this dataset.
```
$ mpttune finetune \
    --model=mpt-7b-storywriter-4bit \
    --weights=./model.safetensors \
